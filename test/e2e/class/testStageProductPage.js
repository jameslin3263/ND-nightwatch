module.exports = {
    ...
    'Stage正常購物流程(未登錄)': browser => {
        ...
        browser //檢查PromotionCard是否有顯示折扣內容
            .useCss()
            .getText('div.hfGNlk',function(res){
                this.expect.element('div.bhCMHZ').text.to.contain(res.value)
            })
        browser //scroll到顯示stickyHeader後檢查 '折扣中'的Tag是否顯示
            .useXpath()
            //.execute('scrollTo(0,2000)')
            .execute(function(scroll){
        ...
        browser
            ...
            .getText('//*[@id="__next"]/div/div/div/section[3]/div/aside/div/div[6]/div[1]/div[2]', function(result){
                this.expect.element('//*[@id="__next"]/div/div/div/section[2]/div/aside/div/div[1]/div[5]').text.be.equal(result.value)
            })
                //點擊 '立即結帳' 後到達結帳頁
            .click('//*[@id="submitBtn"]')
